...
class VerticaDialect(default.DefaultDialect):
    ...
    # def _get_ros_count(self, connection, schema=None, **kw):
    ...
    def get_projection_comment(self, connection, schema=None, **kw):
        ...
        projection_type =sql.text(dedent("""
                SELECT is_super_projection,is_key_constraint_projection,is_aggregate_projection,has_expressions ,projection_name
                FROM v_catalog.projections
                WHERE lower(projection_schema) = '%(schema)s' 
                """ % {'schema': schema}))
        ...
        for data in connection.execute(src):
            # ros_count = data['ros_count']
            ros_count.append({"ROS Count": data[0] , "projection_name" : data[1] }) 
        ...
        for ptype in connection.execute(projection_type):            
            projection_types.append({"is_super_projection": ptype[0], "is_key_constraint_projection": ptype[1], "is_aggregate_projection": ptype[2], "has_expressions": ptype[3], "projection_name": ptype[4] })
        # print(projection_types)  
        projections_type = []
        for ptypes in projection_types: 
            for key, value in ptypes.items():
                # projections_type[{"Projection Type": str(lst[i])}] = value
                if value:
                    print(key)
                    if key == "projection_name":
                        print(value)
                        
           
            # for d in range(len(ptypes)):
            #     print(d)
            #     if ptype[d]:
            #         print(lst[d])
                #     projection_types.append({"Projection Type":str(lst[i])})
        ...
        return {"text": "Vertica physically stores table data in projections, \
            ...
            For more info on projections and corresponding properties check out the Vertica Docs: https://www.vertica.com/docs",
                "properties": {"ROS Count": "something",
